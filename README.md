...
DeepSeekEyes can integrate the Jev API as the fast decision layer for browser and native application control. Jev selects an operation and semantic target; DeepSeekEyes remains the execution and verification layer. Each step is re-observed against the newest `stateId`, text is supplied from local prepared slots (never generated by Jev), low-confidence or risky actions stop for escalation, and `done` is reported as unverified until the host state is checked. Computer Use remains available as the compatibility/fallback executor.

Enable the corresponding Browser/Desktop executor first. Enable Jev in **Settings → Plugins → DeepSeekEyes → Jev control**, or headlessly with `DEEPSEEKEYES_JEV_ENABLED=true` and a `TYPESAFE_API_KEY` environment variable. The `jev` tool defaults to dry-run; pass `execute=true` only when the planned action is acceptable. Jev usage is recorded under the local `jevControl` token category.
