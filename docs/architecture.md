...
| `src/mcp/config.js` / `src/mcp/official-adapter.js` | Strictly validate server definitions and materialize stdio `env` / HTTP-header environment-variable references only when starting the official DSH MCP client. |
...
## MCP 0.6 control plane and execution path

MCP is an opt-in structured application path. DeepSeekEyes loads the official runtime pair `@deepseek-ai/dsh-mcp-client@0.1.0-rc.6` and `@deepseek-ai/dsh-tools@0.1.0-rc.6` from DSH's managed `$DSH_HOME/profiles/node_modules` Host fallback rather than implementing MCP framing, transports or tool-SDK rendering. Each entry is canonicalized and proved to remain inside the managed Host package root, so the current profile's `node_modules` cannot shadow it. The packages are exact optional Host peers and explicit source-test development pins, not installed runtime dependencies: this preserves the Host's Cordis services and symbol-keyed tool scheduler instead of creating a second core graph inside the profile. The adapter runs each configured client inside a Cordis child fiber, captures the official client's registered tool definitions, and disposes the fiber and its stdio child process when a server is disabled, reconfigured or the plugin stops. Supported transports are local `stdio` and remote `streamable-http`.
...
The manager connects only enabled servers when the global `mcpEnabled` switch is on. It discovers tools, exposes health/latency/errors, supports live refresh and reconnect, and responds to runtime reconfiguration without restarting DSH. `testConnection()` performs a real independent transport/tools-list probe, `refresh` replaces the active transport generation, and status polling treats a successful transport operation as fresh for 30 seconds before single-flighting another live probe. A non-empty generation becoming empty withdraws exposure and enters `unknown`; only a matching uncached zero-tool probe establishes a genuinely healthy empty catalog. A failed/mismatched probe remains disconnected instead of reporting captured tools as healthy.
...
This catalog budget is a post-client retention/traversal boundary, not a wire-level pagination budget. Official rc.6 fully drains and validates every `tools/list` page and builds a complete definition map before invoking CaptureRegistry. DeepSeekEyes therefore cannot use this boundary to cap the byte size of one upstream page, the number of cursor pages or rc.6's temporary pre-capture map.

The surviving definitions are registered in the process-global `ctx.tools` registry so generations can be hot-swapped. That registry is not the model-facing authority: the final system-prompt assembly keeps the MCP schemas and guidance only when its selected Provider is the DeepSeekEyes virtual Provider, and strips both for every other Provider. `executeManaged()` repeats the route check and rejects missing-agent as well as wrong-Provider calls before any external invocation. This two-boundary design prevents unrelated model routes from paying MCP Schema context or invoking the managed server.
...
That continuation guard has a narrower boundary than an external-call quota. `automationMaxCallsPerTurn` counts final-model requests for the user instruction; it does not count each MCP function invoked inside a single `run_code`. ToolRuntime currently supplies concurrent-execution control and per-tool timeout, and the manager's optional authorization hook is not wired into the packaged plugin as a per-sub-call approval prompt. Thus 0.6 has no cumulative DeepSeekEyes limit on external MCP calls within one run. Default-off MCP, empty allowlists and server/credential limits are mitigations. The P1 design is `mcpMaxExternalCallsPerRun`, recommended default `64` with explicit `0` unlimited, checked before every managed external invocation and attributed separately from final-model call guards.
...
Each call passes through a staged result boundary. The pinned official rc.6 client and MCP SDK first parse/decode the transport response. Whenever `content` is an array, rc.6 walks its blocks and joins extracted text before checking `isError`; it discards the temporary joined string on success and throws it on failure. DeepSeekEyes then bounds and redacts an already-created exception, but its later successful-value admission cannot limit the dependency's earlier decode or pre-admission extract/join allocation.
...
Reconfiguration and stop set a manager-wide exposure suspension before awaiting any transport close. `syncExposure()` removes all registered MCP definitions and the prompt section while cleanup is in flight; per-runtime close also revokes its generation before awaiting adapter disposal. Reconfiguration republishes only the validated replacement set after all cleanup/connect work finishes, while stop finishes with zero exposure. A close failure is retained as cleanup health state and never restores an old schema.

The 0.6 protocol surface is Tools-only. Resources and Prompts have no Harness consumer in this integration; interactive OAuth, server sandboxing and a general background UI driver are also outside this layer. stdio inherits the `dsh web` process privileges, HTTP behavior depends on the remote server, and the agent must verify an external write from returned/read-back evidence rather than treating transport success as proof.

`npm run test:mcp` exercises the official rc.6 client with temporary SDK servers over both a real stdio child lifecycle and a real loopback Streamable HTTP lifecycle, including connection, discovery, call, probe/refresh and disposal. Package verification and doctor additionally require the exact Host-peer boundary and reject runtime dependencies on the core pair; clean-profile acceptance verifies actual Loader resolution, native tool execution and scheduler identity. These checks validate those local protocol/package paths, not an arbitrary external deployment or TLS certificate.
...
- Catalog bounds apply only after official rc.6 has drained/validated paginated discovery and built its temporary definition map, so they do not bound upstream page bytes or cursor count.
