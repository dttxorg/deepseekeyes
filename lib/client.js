...
// src/mcp/credential-policy.js
var DIRECT_CREDENTIAL_OPTIONS = /* @__PURE__ */ new Set([
  "--access-token",
  "--access-key",
  "--api-key",
  "--apikey",
...
var HEADER_OPTIONS = /* @__PURE__ */ new Set([
  "--header",
  "--headers",
  "--http-header",
  "--proxy-header",
...
var SENSITIVE_HEADER_NAME = /^(?:authorization|proxy-authorization|cookie|set-cookie|api-key|x-api-key|x-auth-token|x-access-token|x-csrf-token|x-goog-api-key|x-amz-security-token)$/i;
var INLINE_AUTH_SCHEME = /(?:^|[:=]\s*)(?:basic|bearer)\s+[A-Za-z0-9._~+/=-]{3,}/i;
var INLINE_CREDENTIAL_ASSIGNMENT = /(?:^|[?&,\s{["'])(?:access[-_]?key|access[-_]?token|api[-_]?key|api[-_]?token|auth|authorization|client[-_]?secret|credential|cookie|oauth2?[-_]?token|pass|password|passwd|pat|private[-_]?key|refresh[-_]?token|secret|secret[-_]?key|session[-_]?token|token)["']?\s*[:=]\s*["']?\S+/i;
var KNOWN_TOKEN_PREFIX = /(?:^|[^A-Za-z0-9])(?:github_pat_|ghp_|gho_|ghu_|ghs_|ghr_|npm_|sk-|sk_)[A-Za-z0-9_-]{8,}/i;
...
function inlineOptionValue(argument) {
  const separator = optionSeparator(argument);
  return separator < 0 ? void 0 : argument.slice(separator + 1);
}
...
function headerOptionValue(argument, nextArgument) {
  ...
  if (!HEADER_OPTIONS.has(name)) return void 0;
...
function sensitiveHeaderValue(value) {
  ...
  return INLINE_AUTH_SCHEME.test(trimmed) || INLINE_CREDENTIAL_ASSIGNMENT.test(trimmed) || KNOWN_TOKEN_PREFIX.test(trimmed);
}
...
function mcpArgsContainInlineCredentials(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (header !== void 0 && sensitiveHeaderValue(header)) return true;
    if (sensitiveStandaloneValue(argument)) return true;
  }
  return false;
}

// src/mcp/url-policy.js
...
function isIpv4Loopback(hostname) {
  ...
  return parts.length === 4 && parts.every((part) => /^(?:0|[1-9]\d{0,2})$/.test(part) && Number(part) <= 255) && Number(parts[0]) === 127;
}
function ipv6Words(hostname) {
  if (!hostname.includes(":") || hostname.includes("%")) return void 0;
  const halves = hostname.split("::");
  if (halves.length > 2) return void 0;
  const left = halves[0] === "" ? [] : halves[0].split(":");
  const right = halves.length === 1 || halves[1] === "" ? [] : halves[1].split(":");
  if ([...left, ...right].some((word) => !/^[0-9a-f]{1,4}$/.test(word))) return void 0;
...
function isExplicitLoopbackHostname(hostname) {
  ...
  return normalized === "localhost" || normalized.endsWith(".localhost") || isIpv4Loopback(normalized) || isIpv6Loopback(normalized);
}
function mcpHttpUrlUsesSecureTransport(url) {
  return url?.protocol === "https:" || url?.protocol === "http:" && isExplicitLoopbackHostname(url.hostname);
}
...
var MCP_SERVER_ID = /^[A-Za-z0-9_-]{1,32}$/;
var ENVIRONMENT_NAME = /^[A-Za-z_][A-Za-z0-9_]*$/;
var HTTP_HEADER_NAME = /^[!#$%&'*+.^_`|~0-9A-Za-z-]+$/;
var CREDENTIAL_QUERY = /(?:api[-_]?key|authorization|credential|password|secret|signature|token)/i;
...
function normalizedReferenceMap(value) {
  ...
  return Object.fromEntries(Object.entries(value).filter(([key, reference]) => typeof key === "string" && typeof reference === "object" && reference !== null && !Array.isArray(reference) && typeof reference.env === "string").map(([key, reference]) => [key.trim(), { env: reference.env.trim() }]).filter(([key]) => key !== ""));
}
function normalizedMcpOAuth(value) {
  const source = value !== null && typeof value === "object" && !Array.isArray(value) ? value : {};
  const reference = (input) => input !== null && typeof input === "object" && !Array.isArray(input) && typeof input.env === "string" && input.env.trim() !== "" ? { env: input.env.trim() } : {};
  return {
...
function createMcpServerDraft(index = 1) {
  return {
    id: `server-${index}`,
...
function nextMcpReferenceEntry(value = {}, { header = false } = {}) {
...
function updateMcpToolSelection(server, tool, selected) {
  const rawName = String(tool?.name ?? tool?.rawName ?? tool?.publicName ?? "");
  ...
  if (!selected) {
    const allowedTools2 = server.allowedTools.filter((selector) => !isExactToolSelector(selector));
    const denyTools2 = server.denyTools.some((selector) => mcpToolMatchesSelector(server, tool, selector)) ? server.denyTools : [...server.denyTools, rawName];
    return { ...server, allowedTools: allowedTools2, denyTools: denyTools2 };
  }
  const denyTools = server.denyTools.filter((selector) => !isExactToolSelector(selector));
  const allowedTools = server.allowedTools.some((selector) => mcpToolMatchesSelector(server, tool, selector)) ? server.allowedTools : [...server.allowedTools, rawName];
...
function mcpContentMatchesSelector(server, kind, item, selector) {
  ...
  const expression = mcpSelectorExpression(selector);
  return [identity, `${server.id}/${identity}`, `${server.name}/${identity}`].some((name) => expression.test(name));
}
function mcpContentAllowedInDraft(server, kind, item) {
...
function updateMcpContentSelection(server, kind, item, selected) {
  const identity = mcpContentIdentity(kind, item);
  const allowField = kind === "resource" ? "allowedResources" : "allowedPrompts";
  const denyField = kind === "resource" ? "denyResources" : "denyPrompts";
  const exactNames = /* @__PURE__ */ new Set([identity, `${server.id}/${identity}`, `${server.name}/${identity}`]);
  const exact = (selector) => !/[?*]/.test(selector) && exactNames.has(selector);
  if (!selected) {
    const allowed2 = server[allowField].filter((selector) => !exact(selector));
    const denied2 = server[denyField].some((selector) => mcpContentMatchesSelector(server, kind, item, selector)) ? server[denyField] : [...server[denyField], identity];
    return { ...server, [allowField]: allowed2, [denyField]: denied2 };
  }
  const denied = server[denyField].filter((selector) => !exact(selector));
  const allowed = server[allowField].some((selector) => mcpContentMatchesSelector(server, kind, item, selector)) ? server[allowField] : [...server[allowField], identity];
  return { ...server, [allowField]: allowed, [denyField]: denied };
}
function normalizeMcpServer(value = {}, index = 0) {
  ...
  return {
    ...
    timeoutMs: Number.isInteger(value.timeoutMs) ? value.timeoutMs : void 0,
...
function normalizeSettingsDraft(value = {}) {
  return {
    ...
    jevEndpoint: typeof value.jevEndpoint === "string" ? value.jevEndpoint : "https://api.typesafe.ai/v1/systemone",
    jevModel: typeof value.jevModel === "string" ? value.jevModel : "jev-latest",
    jevApiKeyEnv: typeof value.jevApiKeyEnv === "string" ? value.jevApiKeyEnv : "TYPESAFE_API_KEY",
    jevMaxSteps: Number.isInteger(value.jevMaxSteps) ? value.jevMaxSteps : 12,
    jevDecisionTimeoutMs: Number.isInteger(value.jevDecisionTimeoutMs) ? value.jevDecisionTimeoutMs : 6e4,
    ...
    mcpServers: Array.isArray(value.mcpServers) ? value.mcpServers.map((server, index) => normalizeMcpServer(server, index)) : [],
    mcpMaxTools: Number.isInteger(value.mcpMaxTools) ? value.mcpMaxTools : 16,
    mcpMaxSchemaTokens: Number.isInteger(value.mcpMaxSchemaTokens) ? value.mcpMaxSchemaTokens : 12e3,
    mcpMaxResultChars: Number.isInteger(value.mcpMaxResultChars) ? value.mcpMaxResultChars : 2e4,
    mcpMaxExternalCallsPerRun: Number.isInteger(value.mcpMaxExternalCallsPerRun) ? value.mcpMaxExternalCallsPerRun : 64,
    mcpToolCallTimeoutMs: Number.isInteger(value.mcpToolCallTimeoutMs) ? value.mcpToolCallTimeoutMs : 3e4,
    mcpAudit: value.mcpAudit !== false,
    mcpArtifactDir: value.mcpArtifactDir === false ? false : typeof value.mcpArtifactDir === "string" ? value.mcpArtifactDir : ""
  };
}
...
function settingsDraftFailure(draft, providerId = "deepseekeyes", upstreamNativeVision = false) {
  ...
  if (!upstreamNativeVision && !draft.autoDetectVision && draft.visionProvider === "" && draft.visionRoutePriority.trim() === "") {
  ...
  for (const server of draft.mcpServers) {
    ...
    if (server.transport === "stdio" && (typeof server.url === "string" && server.url !== "" || Object.keys(server.headers ?? {}).length > 0)) {
      return "mcpServerTransportFields";
    }
    if (server.transport === "streamable-http") {
      if (typeof server.command === "string" && server.command !== "" || typeof server.cwd === "string" && server.cwd !== "" || server.args.length > 0 || Object.keys(server.env ?? {}).length > 0) {
        return "mcpServerTransportFields";
      }
      try {
        ...
        if (url.username !== "" || url.password !== "" || [...url.searchParams.keys()].some((key) => CREDENTIAL_QUERY.test(key))) {
      ...
      if (oauth.enabled) {
        if (typeof oauth.clientId?.env !== "string" || !ENVIRONMENT_NAME.test(oauth.clientId.env) || typeof oauth.clientSecret?.env !== "string" || !ENVIRONMENT_NAME.test(oauth.clientSecret.env)) {
    ...
    if (server.timeoutMs !== void 0 && (!Number.isInteger(server.timeoutMs) || server.timeoutMs < 100 || server.timeoutMs > 36e5)) {
      return "mcpServerTimeoutMsRange";
    }
    if (typeof server.env !== "object" || server.env === null || Array.isArray(server.env) || Object.entries(server.env).some(([key, reference]) => !ENVIRONMENT_NAME.test(key) || typeof reference !== "object" || reference === null || typeof reference.env !== "string" || !ENVIRONMENT_NAME.test(reference.env))) {
      return "mcpServerEnvInvalid";
    }
    if (typeof server.headers !== "object" || server.headers === null || Array.isArray(server.headers) || Object.entries(server.headers).some(([key, reference]) => !HTTP_HEADER_NAME.test(key) || typeof reference !== "object" || reference === null || typeof reference.env !== "string" || !ENVIRONMENT_NAME.test(reference.env))) {
  ...
  const ranges = [
    ...
    ["mcpMaxTools", 0, 1e3],
    ["mcpMaxResultChars", 256, 1e7],
    ["mcpMaxExternalCallsPerRun", 0, 1e4],
    ["mcpToolCallTimeoutMs", 100, 36e5]
  ];
...
function rpcMethodUnavailable(error) {
  ...
  return ["not-found", "not_found", "method_not_found", "unknown_method", "unavailable"].includes(code) || /method.*(not found|unknown)|rpc.*(not found|unavailable)/.test(message);
}
...
function McpMetric({ label, value }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.metric, children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { style: styles.metricValue, children: value ?? "\u2014" }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.metricLabel, children: label })
  ] });
}
function McpReferenceEditor({ id, title, addLabel, value, disabled, header, onChange, t }) {
  ...
  const replaceAt = (index, key, env) => {
    const next = entries.map(([currentKey, reference], currentIndex) => currentIndex === index ? [key, { env }] : [currentKey, reference]);
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.label, children: title }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.referenceList, children: entries.map(([key, reference], index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.referenceRow, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "input",
        {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { id, type: "button", style: styles.button, disabled, onClick: add, children: addLabel }) })
  ] });
}
function McpServerEditor({ server, index, runtimeServer, disabled, busy, onChange, onRemove, onAction, t }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { style: styles.serverCard, "aria-label": `${t("mcpServer")}: ${server.name || server.id}`, children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.serverHeader, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.sectionTitle, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h4", { style: styles.serverHeading, children: server.name || `${t("mcpServer")} ${index + 1}` }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { ...styles.statusBadge, ...mcpStatusStyle(status) }, children: t(mcpStatusLabel(status)) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled, onClick: onRemove, children: t("mcpRemoveServer") })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: server.enabled, disabled, onChange: (event) => onChange({ ...server, enabled: event.target.checked }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpServerEnabled") })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: server.toolsEnabled, disabled: serverDisabled, onChange: (event) => onChange({ ...server, toolsEnabled: event.target.checked }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpToolsEnabled") })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: server.resourcesEnabled, disabled: serverDisabled, onChange: (event) => onChange({ ...server, resourcesEnabled: event.target.checked }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpResourcesEnabled") })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: server.promptsEnabled, disabled: serverDisabled, onChange: (event) => onChange({ ...server, promptsEnabled: event.target.checked }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpPromptsEnabled") })
      ] })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpCapabilitiesHint") }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-risk-policy-${index}`, children: t("mcpRiskPolicy") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "select",
        {
          ...
          children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "allow", children: t("mcpRiskPolicyAllow") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "read-only", children: t("mcpRiskPolicyReadOnly") })
          ]
        }
      ),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpRiskPolicyHint") })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-id-${index}`, children: t("mcpServerId") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-id-${index}`, style: styles.input, type: "text", value: server.id, disabled, onChange: (event) => onChange({ ...server, id: event.target.value }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-name-${index}`, children: t("mcpServerName") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-name-${index}`, style: styles.input, type: "text", value: server.name, disabled, onChange: (event) => onChange({ ...server, name: event.target.value }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-transport-${index}`, children: t("mcpTransport") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
          "select",
          {
            ...
            onChange: (event) => {
              ...
              onChange(transport === "stdio" ? { ...server, transport, url: "", headers: {}, oauth: { ...server.oauth, enabled: false } } : { ...server, transport, command: "", args: [], cwd: "", env: {} });
            },
            children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "stdio", children: t("mcpTransportStdio") }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "streamable-http", children: t("mcpTransportHttp") })
            ]
          }
        )
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-timeout-${index}`, children: t("mcpServerTimeoutMs") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "input",
          {
            ...
            onChange: (event) => onChange({ ...server, timeoutMs: event.target.value === "" ? void 0 : numberFrom(event) })
          }
        )
      ] })
    ] }),
    server.transport === "stdio" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-command-${index}`, children: t("mcpCommand") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-command-${index}`, style: styles.input, type: "text", value: server.command, placeholder: t("mcpCommandPlaceholder"), disabled: serverDisabled, onChange: (event) => onChange({ ...server, command: event.target.value }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-cwd-${index}`, children: t("mcpCwd") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-cwd-${index}`, style: styles.input, type: "text", value: server.cwd, placeholder: t("mcpCwdPlaceholder"), disabled: serverDisabled, onChange: (event) => onChange({ ...server, cwd: event.target.value }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-args-${index}`, children: t("mcpArgs") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-args-${index}`, style: styles.textArea, value: linesOf(server.args), placeholder: t("mcpArgsPlaceholder"), disabled: serverDisabled, onChange: (event) => onChange({ ...server, args: argsFromText(event.target.value) }) })
      ] })
    ] }) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-url-${index}`, children: t("mcpUrl") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-url-${index}`, style: styles.input, type: "url", value: server.url, placeholder: t("mcpUrlPlaceholder"), disabled: serverDisabled, onChange: (event) => onChange({ ...server, url: event.target.value }) })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      server.transport === "stdio" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpReferenceEditor, { id: `deepseekeyes-mcp-env-add-${index}`, title: t("mcpEnvRefs"), addLabel: t("mcpAddEnvRef"), value: server.env, disabled: serverDisabled, onChange: (env) => onChange({ ...server, env }), t }) : null,
      server.transport === "streamable-http" ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpReferenceEditor, { id: `deepseekeyes-mcp-header-add-${index}`, title: t("mcpHeaderRefs"), addLabel: t("mcpAddHeaderRef"), value: server.headers, disabled: serverDisabled, header: true, onChange: (headers) => onChange({ ...server, headers }), t }) : null
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpCredentialHint") }),
    server.transport === "streamable-http" ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { style: { ...styles.details, borderTop: 0, paddingTop: 0 }, open: server.oauth?.enabled === true, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("summary", { style: { ...styles.detailsSummary, marginBottom: 0 }, children: t("mcpOAuthEnabled") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { display: "grid", gap: 12, marginTop: 12 }, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            "input",
            {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpOAuthHint") })
          ] })
        ] }),
        server.oauth?.enabled === true ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-oauth-client-id-${index}`, children: t("mcpOAuthClientIdEnv") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-oauth-client-id-${index}`, style: styles.input, type: "text", value: server.oauth?.clientId?.env ?? "", disabled: serverDisabled, onChange: (event) => onChange({ ...server, oauth: { ...server.oauth, clientId: { env: event.target.value } } }) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-oauth-client-secret-${index}`, children: t("mcpOAuthClientSecretEnv") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-oauth-client-secret-${index}`, style: styles.input, type: "text", value: server.oauth?.clientSecret?.env ?? "", disabled: serverDisabled, onChange: (event) => onChange({ ...server, oauth: { ...server.oauth, clientSecret: { env: event.target.value } } }) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-oauth-scope-${index}`, children: t("mcpOAuthScope") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: `deepseekeyes-mcp-oauth-scope-${index}`, style: styles.input, type: "text", placeholder: t("mcpOAuthScopePlaceholder"), value: server.oauth?.scope ?? "", disabled: serverDisabled, onChange: (event) => onChange({ ...server, oauth: { ...server.oauth, scope: event.target.value } }) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-oauth-method-${index}`, children: t("mcpOAuthAuthMethod") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", { id: `deepseekeyes-mcp-oauth-method-${index}`, style: styles.input, value: server.oauth?.authMethod ?? "client_secret_basic", disabled: serverDisabled, onChange: (event) => onChange({ ...server, oauth: { ...server.oauth, authMethod: event.target.value } }), children: [
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "client_secret_basic", children: t("mcpOAuthAuthBasic") }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: "client_secret_post", children: t("mcpOAuthAuthPost") })
            ] })
          ] })
        ] }) : null
      ] })
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-allow-${index}`, children: t("mcpAllowedTools") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-allow-${index}`, style: styles.textArea, value: linesOf(server.allowedTools), placeholder: t("mcpAllowedToolsPlaceholder"), disabled: serverDisabled || !server.toolsEnabled, onChange: (event) => onChange({ ...server, allowedTools: listFromText(event.target.value) }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-deny-${index}`, children: t("mcpDenyTools") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-deny-${index}`, style: styles.textArea, value: linesOf(server.denyTools), placeholder: t("mcpDenyToolsPlaceholder"), disabled: serverDisabled || !server.toolsEnabled, onChange: (event) => onChange({ ...server, denyTools: listFromText(event.target.value) }) })
      ] })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpToolsHint") }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-resource-allow-${index}`, children: t("mcpAllowedResources") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-resource-allow-${index}`, style: styles.textArea, value: linesOf(server.allowedResources), disabled: serverDisabled || !server.resourcesEnabled, onChange: (event) => onChange({ ...server, allowedResources: listFromText(event.target.value) }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-resource-deny-${index}`, children: t("mcpDenyResources") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-resource-deny-${index}`, style: styles.textArea, value: linesOf(server.denyResources), disabled: serverDisabled || !server.resourcesEnabled, onChange: (event) => onChange({ ...server, denyResources: listFromText(event.target.value) }) })
      ] })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpResourcesHint") }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.grid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-prompt-allow-${index}`, children: t("mcpAllowedPrompts") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-prompt-allow-${index}`, style: styles.textArea, value: linesOf(server.allowedPrompts), disabled: serverDisabled || !server.promptsEnabled, onChange: (event) => onChange({ ...server, allowedPrompts: listFromText(event.target.value) }) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: `deepseekeyes-mcp-prompt-deny-${index}`, children: t("mcpDenyPrompts") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", { id: `deepseekeyes-mcp-prompt-deny-${index}`, style: styles.textArea, value: linesOf(server.denyPrompts), disabled: serverDisabled || !server.promptsEnabled, onChange: (event) => onChange({ ...server, denyPrompts: listFromText(event.target.value) }) })
      ] })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpPromptsHint") }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.metricGrid, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpToolsStatus"), value: t(mcpStatusLabel(runtimeServer?.toolsStatus)) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpContentStatus"), value: t(mcpStatusLabel(runtimeServer?.contentStatus)) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpToolsLatency"), value: Number.isFinite(runtimeServer?.toolsLatencyMs) ? `${formatCount(runtimeServer.toolsLatencyMs)} ms` : "\u2014" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpContentLatency"), value: Number.isFinite(runtimeServer?.contentLatencyMs) ? `${formatCount(runtimeServer.contentLatencyMs)} ms` : "\u2014" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpToolCount"), value: formatCount(runtimeServer?.toolCount) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpResourceCount"), value: formatCount(runtimeServer?.resourceCount) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpResourceTemplates"), value: formatCount(runtimeServer?.resourceTemplateCount) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpPromptCount"), value: formatCount(runtimeServer?.promptCount) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpSelectedCount"), value: formatCount(runtimeServer?.exposedToolCount ?? server.allowedTools.length) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpSchemaTokens"), value: formatCount(runtimeServer?.schemaTokensEstimated) }),
      runtimeServer?.oauth ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpOAuthStatus"), value: runtimeServer.oauth.status ?? "\u2014" }) : null
    ] }),
    runtimeServer?.toolsLastError?.message ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: styles.statusError, children: [
      t("mcpToolsLastError"),
      ": ",
      runtimeServer.toolsLastError.code ? `[${runtimeServer.toolsLastError.code}] ` : "",
      runtimeServer.toolsLastError.message
    ] }) : null,
    runtimeServer?.contentLastError?.message ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: styles.statusError, children: [
      t("mcpContentLastError"),
      ": ",
      runtimeServer.contentLastError.code ? `[${runtimeServer.contentLastError.code}] ` : "",
      runtimeServer.contentLastError.message
    ] }) : null,
    runtimeServer?.lastError?.message && !runtimeServer?.toolsLastError?.message && !runtimeServer?.contentLastError?.message ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: styles.statusError, children: [
      t("mcpLastError"),
      ": ",
      runtimeServer.lastError.code ? `[${runtimeServer.lastError.code}] ` : "",
      runtimeServer.lastError.message
    ] }) : null,
    tools.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.toolList, children: tools.map((tool) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.toolRow, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: mcpToolAllowedInDraft(server, tool), disabled: serverDisabled, onChange: (event) => setAllowed(tool, event.target.checked) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolName, children: tool.name }),
        tool.description ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolDescription, children: tool.description }) : null
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { style: styles.toolMeta, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.statusBadge, children: formatCount(tool.schemaTokensEstimated) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { ...styles.statusBadge, ...tool.exposed ? styles.statusBadgeOk : tool.riskPolicyAllowed === false ? styles.statusBadgeError : tool.allowed ? styles.statusBadgeWarn : void 0 }, children: t(tool.exposed ? "mcpToolExposed" : tool.riskPolicyAllowed === false ? "mcpToolRiskBlocked" : tool.allowed ? "mcpToolAllowedNotExposed" : "mcpToolNotAllowed") })
      ] })
    ] }, tool.name)) }) : null,
    resources.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.toolList, children: resources.map((resource) => {
      const identity = resource.uri ?? resource.uriTemplate;
      return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.toolRow, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: mcpContentAllowedInDraft(server, "resource", resource), disabled: serverDisabled || !server.resourcesEnabled, onChange: (event) => setContentAllowed("resource", resource, event.target.checked) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolName, children: resource.name ?? identity }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolDescription, children: identity })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.statusBadge, children: resource.uriTemplate ? t("mcpResourceTemplates") : "Resource" })
      ] }, identity);
    }) }) : null,
    prompts.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.toolList, children: prompts.map((prompt) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.toolRow, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: mcpContentAllowedInDraft(server, "prompt", prompt), disabled: serverDisabled || !server.promptsEnabled, onChange: (event) => setContentAllowed("prompt", prompt, event.target.checked) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolName, children: prompt.title ?? prompt.name }),
        prompt.description ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.toolDescription, children: prompt.description }) : null
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: styles.statusBadge, children: "Prompt" })
    ] }, prompt.name)) }) : null,
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.actions, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled: busy || !server.enabled, onClick: () => onAction("test", server.id), children: t("mcpTestConnection") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled: busy || !server.enabled, onClick: () => onAction("tools", server.id), children: t("mcpRefreshTools") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled: busy || !server.enabled || !server.resourcesEnabled && !server.promptsEnabled, onClick: () => onAction("content", server.id), children: t("mcpRefreshContent") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled: busy || !server.enabled, onClick: () => onAction("reconnect", server.id), children: t("mcpReconnect") })
    ] })
  ] });
}
function McpSettingsSection({ draft, disabled, rpc, refreshRevision, onUpdate, t }) {
  const [runtime, setRuntime] = (0, import_react.useState)({ loading: true, value: void 0, error: void 0, busy: void 0 });
  const loadRuntime = (0, import_react.useCallback)(async () => {
    setRuntime((current) => ({ ...current, loading: true, error: void 0 }));
    try {
      const value = await callMcpRpc(rpc, "snapshot", {});
      setRuntime({ loading: false, value, error: void 0, busy: void 0 });
    } catch (error) {
      setRuntime({ loading: false, value: void 0, error: messageOf(error), busy: void 0 });
    }
  }, [rpc]);
  (0, import_react.useEffect)(() => {
    void loadRuntime();
  }, [loadRuntime, refreshRevision]);
  ...
  const act = async (method, serverId) => {
    setRuntime((current) => ({ ...current, busy: `${method}:${serverId}`, error: void 0 }));
    try {
      ...
      const value = await callMcpRpc(rpc, "snapshot", {});
      const actionError = action?.ok === false ? `${action.error?.code ? `[${action.error.code}] ` : ""}${action.error?.message ?? "MCP connection test failed"}` : void 0;
      setRuntime({ loading: false, value, error: actionError, busy: void 0 });
    } catch (error) {
      setRuntime((current) => ({ ...current, loading: false, error: messageOf(error), busy: void 0 }));
    }
  };
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { style: styles.details, children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("summary", { style: { ...styles.detailsSummary, marginBottom: 0 }, children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { style: styles.sectionTitle, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpTitle") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { style: styles.version, "aria-label": `${t("mcpVersion")}: ${PLUGIN_VERSION}`, children: [
        "v",
        PLUGIN_VERSION
      ] })
    ] }) }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { display: "grid", gap: 14, marginTop: 14 }, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: draft.mcpEnabled, disabled, onChange: (event) => onUpdate("mcpEnabled", event.target.checked) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
          t("mcpEnabled"),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpEnabledHint") })
        ] })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.statusWarn, children: t("mcpTokenCost") }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { style: { ...styles.details, borderTop: 0, paddingTop: 0 }, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("summary", { style: { ...styles.detailsSummary, marginBottom: 0 }, children: t("mcpBudgets") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { ...styles.grid, marginTop: 14 }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-max-tools", children: t("mcpMaxTools") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-max-tools", style: styles.input, type: "number", min: "0", max: "1000", step: "1", value: draft.mcpMaxTools, disabled, onChange: (event) => onUpdate("mcpMaxTools", numberFrom(event)) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpMaxToolsHint") })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-schema-budget", children: t("mcpMaxSchemaTokens") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-schema-budget", style: styles.input, type: "number", min: "0", max: "10000000", step: "1", value: draft.mcpMaxSchemaTokens, disabled, onChange: (event) => onUpdate("mcpMaxSchemaTokens", numberFrom(event)) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpMaxSchemaTokensHint") })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-result-chars", children: t("mcpMaxResultChars") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-result-chars", style: styles.input, type: "number", min: "256", max: "10000000", step: "1", value: draft.mcpMaxResultChars, disabled, onChange: (event) => onUpdate("mcpMaxResultChars", numberFrom(event)) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpMaxResultCharsHint") })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-external-calls", children: t("mcpMaxExternalCallsPerRun") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-external-calls", style: styles.input, type: "number", min: "0", max: "10000", step: "1", value: draft.mcpMaxExternalCallsPerRun, disabled, onChange: (event) => onUpdate("mcpMaxExternalCallsPerRun", numberFrom(event)) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpMaxExternalCallsPerRunHint") })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-call-timeout", children: t("mcpToolCallTimeoutMs") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-call-timeout", style: styles.input, type: "number", min: "100", max: "3600000", step: "1", value: draft.mcpToolCallTimeoutMs, disabled, onChange: (event) => onUpdate("mcpToolCallTimeoutMs", numberFrom(event)) })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: { ...styles.checkboxRow, marginTop: 14 }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: draft.mcpAudit, disabled, onChange: (event) => onUpdate("mcpAudit", event.target.checked) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
            t("mcpAudit"),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("mcpAuditHint") })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: { ...styles.checkboxRow, marginTop: 12 }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { type: "checkbox", checked: draft.mcpArtifactDir !== false, disabled, onChange: (event) => onUpdate("mcpArtifactDir", event.target.checked ? "" : false) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: t("mcpPersistArtifacts") })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { ...styles.field, marginTop: 12 }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-mcp-artifact-dir", children: t("mcpArtifactDir") }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-mcp-artifact-dir", style: styles.input, type: "text", value: typeof draft.mcpArtifactDir === "string" ? draft.mcpArtifactDir : "", placeholder: t("mcpArtifactDirPlaceholder"), disabled: disabled || draft.mcpArtifactDir === false, onChange: (event) => onUpdate("mcpArtifactDir", event.target.value) })
        ] })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.serverHeader, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { style: styles.serverHeading, children: t("mcpServer") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled, onClick: addServer, children: t("mcpAddServer") })
      ] }),
      draft.mcpServers.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("mcpNoServers") }) : null,
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.serverList, children: draft.mcpServers.map((server, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        McpServerEditor,
        {
          server,
          index,
          ...
          onAction: (method, serverId) => {
            void act(method, serverId);
          },
          t
        },
        index
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.serverHeader, children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { style: styles.serverHeading, children: t("mcpHealth") }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", style: styles.button, disabled: runtime.loading, onClick: () => {
          void loadRuntime();
        }, children: t("mcpRefreshTools") })
      ] }),
      runtime.loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.statusWarn, children: t("mcpRuntimeLoading") }) : null,
      runtime.error !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: styles.statusWarn, children: [
        runtime.value === void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
          t("mcpRuntimeUnavailable"),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {})
        ] }) : null,
        runtime.error
      ] }) : null,
      !runtime.loading && runtime.value !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.metricGrid, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpServer"), value: `${formatCount(runtime.value.summary?.connectedServers)} / ${formatCount(runtime.value.summary?.enabledServers)}` }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpSelectedCount"), value: formatCount(runtime.value.summary?.exposedSchemas ?? runtime.value.summary?.exposedTools) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)(McpMetric, { label: t("mcpSchemaTokens"), value: formatCount(runtime.value.summary?.schemaTokensEstimated) })
        ] }),
        runtime.value.updatedAt ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: styles.hint, children: [
          t("mcpRuntimeUpdatedAt"),
...
function DeepSeekEyesSettingsCard({ scope, api, usageRpc, t }) {
  ...
  const [mcpRuntimeRefreshRevision, setMcpRuntimeRefreshRevision] = (0, import_react.useState)(0);
  (0, import_react.useEffect)(() => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("li", { style: { ...styles.card, ...open ? styles.cardOpen : {} }, children: [
    ...
    open ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { id: "deepseekeyes-settings-body", style: styles.body, children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.statusOk, children: upstreamNativeVision ? routeSummary(t("nativeRouteSummary"), visionRouteName, finalRouteName) : routeSummary(t("routeSummary"), visionRouteName, finalRouteName) }),
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { style: styles.details, open: true, children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
            t("jevEnabled"),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { style: styles.hint, children: t("jevEnabledHint") })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: { ...styles.grid, marginTop: 14 }, children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-jev-endpoint", children: t("jevEndpoint") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-jev-endpoint", style: styles.input, type: "url", value: draft.jevEndpoint, disabled: saving || !snapshot.writable || !draft.jevEnabled, onChange: (event) => update("jevEndpoint", event.target.value) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-jev-model", children: t("jevModel") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-jev-model", style: styles.input, type: "text", value: draft.jevModel, disabled: saving || !snapshot.writable || !draft.jevEnabled, onChange: (event) => update("jevModel", event.target.value) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-jev-key-env", children: t("jevApiKeyEnv") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-jev-key-env", style: styles.input, type: "text", value: draft.jevApiKeyEnv, disabled: saving || !snapshot.writable || !draft.jevEnabled, onChange: (event) => update("jevApiKeyEnv", event.target.value) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-jev-max-steps", children: t("jevMaxSteps") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-jev-max-steps", style: styles.input, type: "number", min: "1", max: "30", step: "1", value: draft.jevMaxSteps, disabled: saving || !snapshot.writable || !draft.jevEnabled, onChange: (event) => update("jevMaxSteps", numberFrom(event)) })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.field, children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("label", { style: styles.label, htmlFor: "deepseekeyes-jev-timeout", children: t("jevDecisionTimeoutMs") }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", { id: "deepseekeyes-jev-timeout", style: styles.input, type: "number", min: "1000", max: "120000", step: "1", value: draft.jevDecisionTimeoutMs, disabled: saving || !snapshot.writable || !draft.jevEnabled, onChange: (event) => update("jevDecisionTimeoutMs", numberFrom(event)) })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { style: styles.divider }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", { style: styles.checkboxRow, children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("details", { style: styles.details, open: true, children: [
        ...
        usage.loading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: { ...styles.statusWarn, marginTop: 12 }, children: t("usageStatsLoading") }) : usage.error !== void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { style: { ...styles.statusError, marginTop: 12 }, children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { style: styles.metricGrid, children: [
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpTokens"), value: usage.value.totals.derived.mcpTokens }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpExternalCalls"), value: usage.value.totals.mcpExternalCalls }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpSchemaInput"), value: usage.value.totals.mcpSchemaInputTokensEstimated }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpResultInput"), value: usage.value.totals.mcpResultInputTokensEstimated }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpContextCompactions"), value: usage.value.totals.mcpContextCompactions }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageMcpLimitStops"), value: usage.value.totals.mcpLimitStops }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageVisualTurns"), value: usage.value.totals.visualTurns }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UsageMetric, { label: t("usageNativeVisualTurns"), value: usage.value.totals.nativeVisualTurns }),
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { style: styles.hint, children: t("usageMcpSubsetHint") }),
