...
test('native desktop driver routes macOS and Windows to packaged native helpers', async () => {
  ...
  assert.equal(calls[1].input.semantic, true)
  assert.equal(calls[1].input.maxElements, 200)
...
test('packaged helpers retain both native platforms and avoid the macOS CFRelease crash', async () => {
  ...
  assert.match(mac, /function resolveTypeTargetWindow\(input\)/)
  assert.match(mac, /function assertTypeTargetWindow\(target, fallback, targetElement, clickPoint\)/)
  assert.match(mac, /String\(current\.nativeId\) === `cg:\$\{processFrontmostWindow\.number\}`/)
  assert.match(mac, /&& windowMatches\s+&& pointMatches/)
  assert.match(mac, /targetMode = 'screenshot-coordinate'/)
  assert.match(mac, /TARGET_FOCUS_MISMATCH:/)
  assert.match(mac, /text was not sent/)
  assert.match(mac, /focusProcessByPid\(target\.info\.pid\)/)
  assert.match(mac, /function snapshotPasteboard\(board\)/)
  assert.match(mac, /function restorePasteboard\(board, records\)/)
  assert.match(mac, /function insertAxSelectedText\(element, text\)/)
  assert.match(mac, /inputMethod/)
  assert.ok(
    mac.indexOf('assertTypeTargetWindow(target, fallback, undefined, clickPoint)')
      < mac.lastIndexOf('clickAt(input, input.x, input.y, $.kCGMouseButtonLeft, 1)'),
  )
  ...
  assert.match(windows, /\[Console\]::OutputEncoding = \$script:DeepSeekEyesUtf8/)
  assert.match(windows, /\$absoluteX = \[int\]\(\$originX \+ \$relativeX\)/)
  assert.match(windows, /return \[int\[\]\]@\(\$absoluteX, \$absoluteY\)/)
  assert.match(windows, /function Resolve-TypeTargetWindow\(\$InputObject\)/)
  assert.match(windows, /function Assert-TypeTargetWindow\(\$TargetWindow, \[string\]\$FallbackNativeId\)/)
  assert.match(windows, /\$targetMode = 'screenshot-coordinate'/)
  assert.match(windows, /TARGET_FOCUS_MISMATCH:/)
  assert.match(windows, /text was not sent/)
  assert.ok(
    windows.indexOf('Assert-TypeTargetWindow $targetWindow $focusedFallback | Out-Null')
      < windows.indexOf('Invoke-LeftClick $point'),
  )
  assert.match(windows, /\$actionResult = Invoke-Action \$inputObject \$actionScreen/)
  assert.doesNotMatch(windows, /\[int\]\$Screen\.X \+ \[int\]/)
})
